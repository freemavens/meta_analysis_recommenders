...
library('fastDummies')
library(data.table)
#setwd('C:/Users/Tom/OneDrive - msqpartners.com/Documents/GitHub/meta-analysis-recommenders/Base_Recommender_System')
...
shinyServer(function(input, output, session) {
      ...
      for(i in 1:nrow(possible_variables)) {  
        ...
        if (variable != selected_variable){
          ...
          if (new_prediction > predicted_score()+0.1){
            ...
            return(HTML(paste0("<br/><p>This score can be improved by changing the ",'<span style="font-size:24px;font-weight:bold;text-decoration:underline;">', selected_column, '</span>', " feature to ",'<span style="font-size:28px;font-weight:bold;text-decoration:underline;">', new_variable, '</span><p/>')))
          }
        }
      }
    }
  ...
  output$prediction = renderUI({
    HTML(paste("<br/>The Average Behaviour Change Score you can expect to obtain by producing an ad with the selected attributes is:<br/>", '<p style="font-size:30px;font-weight:bold;"<b/>',round(prediction(), 2), "<b/><p/>"))
  })
  output$percentile = renderUI({
    HTML(paste("This equates to the ", percentile_string(), " Percentile. As in, there are ", 100-round(percentile(), 2)*100, "% of possible ads that would result in a higher predicted score. The performance of your proposed ad would therefore be classed as:<br/>",'<p style="font-size:28px;font-weight:bold;"<b/>', ad_class(), "<b/><p/>", sep = ''))
  })
